...
from Mound.util import XDGCONFIGDIRS, XDGDATADIRS, LANG_FULL, LANG_SHORT, USERDATA_UPDATE_URL
...
class UserData:
    ...
    def load_defaults(self, scan_directories=default_dirs):
        ...
        for sdir in scan_directories:
            ...
            try:
                self.default_locations.load_from_file(userdata_file)
            except: pass
...
class DefaultsParser(dict):
    ...
    def load_from_file(self, filename):
        with open(filename, 'r') as f:
            self.load_file_object(f)

    def load_from_url(self, url=USERDATA_UPDATE_URL):
        f = urlopen(url)
        self.load_file_object(f)
        f.close()
        
    def load_file_object(self, f):
        """
        Load defaults from a file object and merge with this list.
        """
        loaded = {}
        for line in f:
            line = line.strip()
            if len(line) > 0 and line[0] != '#':
                ...
                if appname in loaded:
                    raise DuplicateDefaultError, "\"%s\" is listed more than once" % appname
                loaded[appname] = locations
        self.update(loaded)

    def write_to_file(self, filename):
        """
        Write the contents of this defaults dictionary to a userdata file
        that can be loaded later.
        """

        with open(filename, 'w') as f:
            f.write("# automatically generated by mound-data-manager\n\n")
            for entry in self:
                f.write("%s %s\n" % (entry, self[entry]))
